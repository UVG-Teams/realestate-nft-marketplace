			...
			{ active || authToken ? (
				<>
					<Tag colorScheme="blue" borderRadius="full">
						<TagLabel>
							<Link to={`/marketplace?address=${account}`}>{ truncatedAddress }</Link>
						</TagLabel>
						<Badge
							d={{
								base: "none",
		...
		register(account, disconnect) {
			...
			if (password != null){
				if (password.length < 6){
					disconnect()
					alert("Password must contain 6 characters minimum")
					return
				} else if (password.search(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)./) == -1){
					disconnect()
					alert("Password must cotain lower case, upper case and number")
					return
				}
			}
