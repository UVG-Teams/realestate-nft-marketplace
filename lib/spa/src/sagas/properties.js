...
function* getData(action) {
    ...
    if (http.isSuccessful(response.httpStatus)) {
        ...
        const filesResponse = yield http.request(
            'GET',
            `/api/properties/${response.id}/retrieve_files.json`,
            null,
            token
        )

        if (http.isSuccessful(filesResponse.httpStatus)) {
            yield put(properties.actions.completedGettingFiles(filesResponse.images))
        } else {
            yield put(properties.actions.failedGettingFiles(filesResponse.msg))
        }
